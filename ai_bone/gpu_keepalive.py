...
컨테이너로 실행 (모든 GPU를 노출 → 스크립트에서 번호로 선택):
  # 1) 현재 상태만 확인
  sudo docker run --rm --gpus all bone-nnunet:2.8.1 \
    python /data1/bone/ai_bone/gpu_keepalive.py
  # 2) 예: GPU 5,6,7 에 부하 (백그라운드, 이름 gpu-keepalive)
  sudo docker run -d --name gpu-keepalive --gpus all -v /data1:/data1 \
    bone-nnunet:2.8.1 python /data1/bone/ai_bone/gpu_keepalive.py 5 6 7

인자   : 대상 GPU 번호 (예:  5 6 7   또는   5,6,7)
환경변수: KEEP_N(행렬크기, 기본 16384) · KEEP_SLEEP_MS(부하 낮추기, 기본 0)
종료   : sudo docker stop gpu-keepalive   (또는 Ctrl+C)
"""
import os
